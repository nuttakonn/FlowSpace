...
public record ExportBoardCommand(Guid BoardId, string Format, string JwtToken = "") : ICommand<ExportResponse>;
    ...
    public async Task<Result<ExportResponse>> Handle(ExportBoardCommand command, CancellationToken cancellationToken)
    {
        ...
        switch (command.Format.ToLower())
        {
            ...
            case "drawio":
                ...
                break;
            case "png":
                data = await _exportService.ExportToPngAsync(command.BoardId, command.JwtToken, cancellationToken);
                contentType = "image/png";
                fileName += ".png";
                break;
            case "jpg":
            case "jpeg":
                data = await _exportService.ExportToJpgAsync(command.BoardId, command.JwtToken, cancellationToken);
                contentType = "image/jpeg";
                fileName += ".jpg";
                break;
            case "pdf":
                data = await _exportService.ExportToPdfAsync(command.BoardId, command.JwtToken, cancellationToken);
                contentType = "application/pdf";
                fileName += ".pdf";
                break;
            case "svg":
                var svgContent = await _exportService.ExportToSvgAsync(command.BoardId, command.JwtToken, cancellationToken);
